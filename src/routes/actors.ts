...
export const register = (router: express.Router) => {
    ...
    router.post('/actors', upload.single('actorPhoto'), joiMiddleware(schemas.actor), asyncMiddleware(controller.postActor));
    router.delete('/actors/:id', asyncMiddleware(controller.deleteActor));
    router.put('/actors/:id', upload.single('actorPhoto'), asyncMiddleware(controller.updateActor));
};